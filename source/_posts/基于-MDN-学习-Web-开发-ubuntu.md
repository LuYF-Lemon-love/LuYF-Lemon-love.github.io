...
{% label HTML 到底是什么？ pink %}

>HTML 不是一门编程语言，而是一种用于**定义内容结构**的标记语言。HTML 由一系列的**元素（elements）**组成，**这些元素可以用来包围不同部分的内容，使其以某种方式呈现或者工作**。**一对标签（ tags）可以为一段文字或者一张图片添加超链接，将文字设置为斜体，改变字号，等等**。例如，键入下面一行内容：
