---
title: Argparse 教程学习笔记-ubuntu
tags:
  - Python
categories: Python
description: argparse --- 命令行选项、参数和子命令解析器.
cover: 'https://cos.luyf-lemon-love.space/images/005-白丝-美女.png'
abbrlink: 2892258040
date: 2023-02-12 21:30:34
---

## 前言

这篇教程旨在作为 [argparse](https://docs.python.org/zh-cn/3/library/argparse.html#module-argparse) 的入门介绍，此模块是 Python 标准库中推荐的命令行解析模块。

>**备注:** 还有另外两个模块可以完成同样的任务，称为 `getopt` (对应于 C 语言中的 `getopt()` 函数) 和被弃用的 `optparse`。还要注意 `argparse` 是基于 `optparse` 的，因此用法与其非常相似。

源教程地址: https://docs.python.org/zh-cn/3/howto/argparse.html .

Argparse 标准库文档地址: https://docs.python.org/3/library/argparse.html .

操作系统：**Ubuntu 20.04.5 LTS**

## 参考文档

1. [Argparse 教程](https://docs.python.org/zh-cn/3/howto/argparse.html)
...
- 如果我们想要使用比它默认提供的更多功能，我们需要告诉该命令更多信息。在这个例子里，我们想要查看一个不同的目录，`pypy`。我们所做的是指定所谓的**位置参数**。之所以这样命名，是因为程序应该如何处理该参数值，完全取决于它在命令行出现的位置。更能体现这个概念的命令如 `cp`，它最基本的用法是 `cp SRC DEST`。第一个位置参数指的是*你想要复制的*，第二个位置参数指的是*你想要复制到的位置*。
...
到目前为止，我们一直在使用 `argparse.ArgumentParser` 实例的两个方法。 让我们再介绍第三个方法 `add_mutually_exclusive_group()`。 它允许我们**指定彼此相互冲突的选项**。 让我们再更改程序的其余部分以便使用新功能更有意义：我们将引入 `--quiet` 选项，它将与 `--verbose` 正好相反:
...
请注意用法文本中有细微的差异。 注意 `[-v | -q]`，它的意思是说我们可以使用 `-v` 或 `-q`，但**不能同时使用两者**：
