...
**DRKG** 是一个由亚马逊上海 **AI** 实验联合多家科研机构**开源的综合生物知识图谱**，涉及**基因**、**药物**、**疾病**、**生物学过程**、**副作用和症状**。**DRKG** 包括 **DrugBank**、**Hetionet**、**GNBR**、**String**、**IntAct** 和 **DGIdb** 六个现有数据库的信息，以及**近期新冠病毒的相关医学文献中挖掘的数据**。**DRKG** 知识图谱包含属于 **13** 种实体类型的 **97,238** 个实体，以及属于 **107** 种关系类型的 **5,874,261** 个三元组数据。同一实体对之间可能存在多种交互类型。其中药物有 **24,313** 个实体，疾病有 **5,103** 个实体，其中头尾实体为药物与疾病的三元组个数为 **83,895**，药物与疾病之间有 **10** 种关系。
...
**药物再利用知识图谱** (Drug Repurposing Knowledge Graph, DRKG) 是一个涉及**基因**、**药物**、**疾病**、**生物过程**、**副作用**和**症状**的**综合生物知识图谱**. **DRKG** 包括来自 **DrugBank**、**Hetionet**、**GNBR**、**String**、**IntAct** 和 **DGIdb** 等**六个现有数据库的信息**, 以及从最近特别与 **Covid19** 相关的出版物中收集的数据. 它包含属于 13 种实体类型的 97,238 个实体; 以及属于 107 种关系类型的 5,874,261 个三元组. 这 107 种关系类型显示了 13 种实体类型之间的交互类型（同一实体对之间可能存在多种交互类型), 如下图所示. 它还包括一堆关于如何使用**统计方法**或使用*机器学习方法*（如**知识图谱嵌入**）探索和分析 DRKG 的笔记本.
...
为了分析 **DRKG**, 你能使用下面的命令直接下载 **drkg**:

```shell
wget https://dgl-data.s3-us-west-2.amazonaws.com/dataset/DRKG/drkg.tar.gz
```

如果你使用仓库提供的 notebooks, 不需要手动下载 drkg, 因为 notebooks 会自动下载.

解压 **drkg.tar.gz**, 会获得下面的文件:
...
- **drkg.tsv**, 一个包含原始 **drkg** 三元组集合 (格式为 **(h, r, t)**) 的 **tsv** 文件.

- **embed**, 一个包含使用全部 **drkg.tsv** 文件为训练集的预训练知识图谱嵌入和来自 [molecule SMILES](./drugbank_info/drugbank_smiles.txt) 的预训练 GNN 分子嵌入的目录.

- **entity2src.tsv**, 一个映射 drkg 中的实体到源数据的文件.

- **relation_glossary.tsv**, 一个包含 DRKG 中关系的术语集的文件，以及其他与源相关的信息 (如果可得到).

#### Pretrained DRKG embedding

这 DRKG 嵌入是使用 400 维度的 TransE\_l2 模型训练的, 有 4 个文件:

- **DRKG\_TransE\_l2\_entity.npy**, 二进制的 **NumPy** 的数据, 包含实体嵌入.
...
为了使用**预训练嵌入**, 可以使用 **np.load** 函数单独**加载实体嵌入**和**关系嵌入**:
...
也使用预训练的 GNN 提供 DrugBank 的大多数小分子药物的分子嵌入. 尤其, [Strategies for Pre-training Graph Neural Networks](https://arxiv.org/abs/1905.12265) 将监督的分子性质预测与自我监督学习结合, 开发了多种方法用于预训练基于 **GNN** 的分子表示. 使用 [DGL-LifeSci](https://github.com/awslabs/dgl-lifesci/tree/master/examples/molecule_embeddings) 计算四种分子嵌入的变体:

- **mol_contextpred.npy**: 一个预训练模型预测分子子图的周围图结构.

- **mol_infomax.npy**：一个预训练模型最大化本地节点表示和全局图谱表示间的相互信息.

- **mol_edgepred.npy**: 一个预训练模型鼓励附近节点有类似的表示和强迫不相干的节点有不同的表示.

- **mol_masking.npy**: 一个预训练模型预测随机遮蔽节点和边的属性.

### Tools to analyze DRKG

使用深度学习框架分析 **DRKG**, 包括 [DGL](https://github.com/dmlc/dgl) (一个图神经网络的框架) 和 [DGL-KE](https://github.com/awslabs/dgl-ke) (一个计算知识图谱嵌入的库).

请按照下面的介绍安装深度学习框架.

#### Install PyTorch

所有的 **notebooks** 都使用 PyTorch 作为深度学习后端.

可以去 [Pytorch 官网](https://pytorch.org/) 安装其他版本.
